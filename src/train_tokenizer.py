...
"""
I don't remember if everything is ok...
...
def update_nllb_tokenizer(old_tokenizer: T5Tokenizer, new_spm_path: str) -> T5Tokenizer:
    """
    ...
    (TKN_DIR / "spiece.model").unlink()

    # Copy new SentencePiece model
    shutil.copy(new_spm_path, TKN_DIR / "spiece.model")

    # Load new tokenizer with additional language codes
    new_tokenizer = T5Tokenizer.from_pretrained(
        TKN_DIR,
    )
...
if __name__ == "__main__":
    ...
    tokenizer_old = T5Tokenizer.from_pretrained(MODEL_ID)
    tokenizer = update_nllb_tokenizer(tokenizer_old, NEW_SPM_NAME)
    ...
    for t in tqdm(added_vocab):
        ...
        model.shared.weight.data[idx] = model.shared.weight.data[tt].mean(0)
